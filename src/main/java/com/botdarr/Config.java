...
public class Config {
  ...
  public static List<StatusEndPoint> getStatusEndpoints() {
    ...
    if (isLidarrEnabled()) {
      statusEndPoints.add(getDomain(Constants.LIDARR_URL, "lidarr"));
    }
    if (isRadarrEnabled()) {
      statusEndPoints.add(getDomain(Constants.RADARR_URL, "radarr"));
    }
    if (isLidarrEnabled()) {
      statusEndPoints.add(getDomain(Constants.SONARR_URL, "sonarr"));
    }
